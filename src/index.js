...
const CONTRACT_ADDRESS = "0x983fF91817Cb9CFE2E151Bbd491ECDd70c81Ca4b";
const DEPLOYMENT_BLOCK = 7145626;
...
function showActiveGame (gameHost){
    contract.methods.games(gameHost).call().then(game=>{
        ...
        gameLog.textContent = `You have ${isHosting? "initiated a game joined by" : "joined a game with"} ${isHosting ? game.visitor : gameHost} for ${web3.utils.fromWei(game.bet, "ether")} ${UNIT}`;
...
function drawGame(gameHost, hasTurn){
    contract.methods.getGame(gameHost).call().then(game=>{
...
function updateGamesList(){
    const p1 = contract.getPastEvents("Created", {fromBlock: DEPLOYMENT_BLOCK, toBlock: "latest"});
    const p2 = contract.getPastEvents("Cancelled", {fromBlock: DEPLOYMENT_BLOCK, toBlock: "latest"});
    const p3 = contract.getPastEvents("Started", {fromBlock: DEPLOYMENT_BLOCK, toBlock: "latest"});
    Promise.all([p1, p2, p3]).then(values=>{
        ...
        if(gamesList[currentAddress] !== undefined && gamesList[currentAddress].length > 0){
            ...
            offer.textContent = `Waiting for a player to join for ${web3.utils.fromWei( gamesList[currentAddress][0], "ether")} ${UNIT}`;
...
function init (){
    ...
    // Contract
    fetch("./build/contracts/TicTacToe.json").then(r=>r.json()).then(compiledContract=>{
...
function handleContract(){
    create.onclick = ()=>{ contract.methods.create().send({from: currentAddress, value: web3.utils.toWei(String(currentBet), "ether")}) };
